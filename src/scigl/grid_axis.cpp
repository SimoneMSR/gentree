...
Grid_Axis::Grid_Axis() :
	...
	set_start(Position(-.5f, 0.5f, 0.5f));
	set_end(Position(-.5f, 0.5f, -.5f));
	set_thickness(2.0f);
	set_orientation(1);
...
void Grid_Axis::render(void) {
	...
	glColor4f(get_fg_color().r, get_fg_color().g, get_fg_color().b,
			get_fg_color().a * get_alpha());
	glPushMatrix();
	glTranslatef(start_.x, start_.y, start_.z);
	glRotatef(az, 0, ry, rz);
	glRotatef(orientation_, 1, 0, 0); // Set ticks orientation
	...
	FontPtr font = FontPtr(Font::Font12());
...
void Grid_Axis::set(float min, float max, int major_n, int minor_n,
		float major_size, float minor_size, float major_thickness,
	...
	for (int i = 0; i < major_n; i++) {
		...
		oss << (int) (min + (max - min) * (i / float(major_n - 1)));
